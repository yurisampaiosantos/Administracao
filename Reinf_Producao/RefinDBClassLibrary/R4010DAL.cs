...
{
    public class R4010DAL : RBASEDAL
    {
        ComplyOraDB c_odb = new ComplyOraDB();

        public override void delete(int idReg)
        {
            String sql = "delete from R4010 where idReg = " + idReg.ToString();
            odb.executeSql(sql);
        }
        public void deleteNFS(int idReg)
        {
            String sql = "delete from R4010_NFS where idReg = " + idReg.ToString();
            odb.executeSql(sql);
        }
        public void deleteNFSServ(int idReg)
        {
            String sql = "delete from R4010_INFOTPSERV where idReg = " + idReg.ToString();
            odb.executeSql(sql);
        }
        public void gerarR4010Comply(int idRegEmpresa, string perApur)
        {
            ...
            string sql =
                    ...
                    "     (select m.bmz_codigo from bsc_matriz m where m.cus_id = (select f.cus_id from bsc_filial f where f.bus_id = s.BUS_ID)) = " + empresa;
            dt4010 = c_odb.getDataTable(sql);

            foreach (DataRow dr in dt4010.Rows)
            {
        ...
        public void gerarEventoExclusao(string cnpj, string nrRecibo, string perApur)
        {
            ...
            r.incluir(cnpj, "R-4010", nrRecibo, perApur);
        }

        public void limparR4010Temp()
        {
            string sqlDelete = "delete from r4010_temp";
            odb.executeSql(sqlDelete);

        }

        public void carregaR4010Temp(DataSet ds)
        {

            limparR4010Temp();
            string sql = "insert into r4010_temp (perapur, empresa, filial, dtemissaonf, vlrretencao,  cpf_cnpjprestador, vlrbruto, numdocto, serie, vlrBaseRet, tpServico, vlrNRetPrincipal, vlrServico15, vlrServico20, vlrServico25, vlrAdicional, vlrNRetAdicional ) " +
        ...
        public void preencherR4010()
        {
            ...
            DataSet ds4010 = new DataSet();
            da.Fill(ds4010);

            // Preencher com os dados da Carga
            string sqlInsR4010 = "insert into r4010 (id, indretif,perapur, tpamb, procemi, verproc, tpinsc, nrinsc, tpinscestab, nrinscestab, indobra) " +
                " values (:id, :indretif, :perapur, :tpamb, :procemi, :verproc, :tpinsc, :nrinsc, :tpinscestab, :nrinscestab, :indobra) " +
                 " RETURNING idReg INTO :idReg";

            OracleCommand cmd = new OracleCommand(sqlInsR4010, oc);      
 
            // Inserir no Evento R4010
            foreach (DataRow r in ds4010.Tables[0].Rows)
            {
                cmd.Parameters.Clear();
                ...
                cmd.ExecuteNonQuery();
                int idReg4010 = Convert.ToInt16(cmd.Parameters["idReg"].Value.ToString());

                // Inserir no Evento R4010_IDEPRESTSERV
                string sql4010_IdePrestServ = "select CPF_CNPJPRESTADOR,  sum(VLRBRUTO) vlrTotalBruto, sum(VLRRETENCAO) vlrTotalRetPrinc, " +
                                        ...
                                        " from R4010_TEMP where empresa = '" + r["empresa"].ToString() + "' and filial = '" + r["filial"].ToString() + "' and PERAPUR = '" + r["perapur"].ToString() + "' and CPF_CNPJPRESTADOR = '" + r["cpf_cnpjprestador"].ToString() + "'" +
                                        " group by CPF_CNPJPRESTADOR";
                da = new OracleDataAdapter(sql4010_IdePrestServ, oc);
                DataSet ds4010IdePrestServ = new DataSet();
                da.Fill(ds4010IdePrestServ);


                string sqlInsR4010_IDEPRESTSERV = "insert into r4010_IDEPRESTSERV (cnpjprestador, VLRTOTALBRUTO, VLRTOTALBASERET, VLRTOTALRETPRINC, VLRTOTALRETADIC, VLRTOTALNRETPRINC, VLRTOTALNRETADIC, indcprb,r4010_idreg) " +
                    " values (:cnpjprestador, :VLRTOTALBRUTO, :VLRTOTALBASERET, :VLRTOTALRETPRINC, :VLRTOTALRETADIC, :VLRTOTALNRETPRINC, :VLRTOTALNRETADIC, :indcprb, :r4010_idreg) " +
                     " RETURNING idReg INTO :idRegIdePrestServ";
                OracleCommand cmdIdePrestServ = new OracleCommand(sqlInsR4010_IDEPRESTSERV, oc);
                foreach (DataRow rIdePrestServ in ds4010IdePrestServ.Tables[0].Rows)
                {
                    ...
                    cmdIdePrestServ.Parameters.Add(new OracleParameter("r4010_idreg", OracleDbType.Int16, ParameterDirection.Input));
                    ...
                    cmdIdePrestServ.Parameters["r4010_idreg"].Value = idReg4010;
                    ...
                    // Inserir no Evento R4010_NFS
                    string sql4010_NFS = "select * from R4010_TEMP where empresa = '" + r["empresa"].ToString() + "' and filial = '" + r["filial"].ToString() + "' and PERAPUR = '" + r["perapur"].ToString() + "' " +
                                         " and CPF_CNPJPRESTADOR = '" + rIdePrestServ["CPF_CNPJPRESTADOR"].ToString() + "'";
                    da = new OracleDataAdapter(sql4010_NFS, oc);
                    DataSet ds4010NFS = new DataSet();
                    da.Fill(ds4010NFS);

                    string sqlInsR4010_NFS = "insert into r4010_NFS (SERIE,NUMDOCTO,DTEMISSAONF,VLRBRUTO, OBS, R4010_IDEPRESTSERV_IDREG) " +
                        " values (:SERIE,:NUMDOCTO,:DTEMISSAONF,:VLRBRUTO, :OBS, :R4010_IDEPRESTSERV_IDREG) " +
                         " RETURNING idReg INTO :idRegNFS";
                    OracleCommand cmdNFS = new OracleCommand(sqlInsR4010_NFS, oc);
                    foreach (DataRow rNFS in ds4010NFS.Tables[0].Rows)
                    {
                        ...
                        cmdNFS.Parameters.Add(new OracleParameter("R4010_IDEPRESTSERV_IDREG", OracleDbType.Int64, ParameterDirection.Input));
                        ...
                        cmdNFS.Parameters["R4010_IDEPRESTSERV_IDREG"].Value = idRegIdePrestServ;
                        cmdNFS.ExecuteNonQuery();
                        int idRegNFS = Convert.ToInt16(cmdNFS.Parameters["idRegNFS"].Value.ToString());

                        // Inserir no Evento R4010_INFOTPSERV
                        string sql4010_INFOTPSERV = "select * from R4010_TEMP where empresa = '" + r["empresa"].ToString() + "' and filial = '" + r["filial"].ToString() + "' and PERAPUR = '" + r["perapur"].ToString() + "' " +
                         " and CPF_CNPJPRESTADOR = '" + rIdePrestServ["CPF_CNPJPRESTADOR"].ToString() + "' and numdocto = '" + rNFS["numdocto"].ToString() + "'";
                        da = new OracleDataAdapter(sql4010_INFOTPSERV, oc);
                        DataSet ds4010InfoTpServ = new DataSet();
                        da.Fill(ds4010InfoTpServ);
                        string sqlInsR4010__INFOTPSERV = "insert into r4010_INFOTPSERV (TPSERVICO, VLRBASERET, VLRRETENCAO,VLRRETSUB,VLRNRETPRINC,VLRSERVICOS15,VLRSERVICOS20,VLRSERVICOS25,VLRADICIONAL,VLRNRETADIC,R4010_NFS_IDREG ) " +
                            " values (:TPSERVICO, :VLRBASERET, :VLRRETENCAO,:VLRRETSUB,:VLRNRETPRINC,:VLRSERVICOS15,:VLRSERVICOS20,:VLRSERVICOS25,:VLRADICIONAL,:VLRNRETADIC,:R4010_NFS_IDREG) " +
                             " RETURNING idReg INTO :idRegINFOTPSERV";
                        OracleCommand cmdINFOTPSERV = new OracleCommand(sqlInsR4010__INFOTPSERV, oc);
                        foreach (DataRow rNFOTPSERV in ds4010InfoTpServ.Tables[0].Rows)
                        {
                            ...
                            cmdINFOTPSERV.Parameters.Add(new OracleParameter("R4010_NFS_IDREG", OracleDbType.Int64, ParameterDirection.Input));
                            ...
                            cmdINFOTPSERV.Parameters["R4010_NFS_IDREG"].Value = idRegNFS;
        ...
        public void ajustarValoresTotais(int idReg)
        {
            string sqlValores = "select " +
                 ...
                 " from R4010_INFOTPSERV where R4010_NFS_IDREG in " +
                 " (select idReg from R4010_NFS where R4010_IDEPRESTSERV_IDREG in (select idReg from R4010_IDEPRESTSERV where R4010_IDREG = " + idReg.ToString() + "))";

            string sqlValorBruto = "select sum(vlrBruto) totalBruto from R4010_NFS where R4010_IDEPRESTSERV_IDREG in (select idReg from R4010_IDEPRESTSERV where R4010_IDREG =  " + idReg.ToString() + ")";
            ...
            string sqlUpdR4010 = "update r4010_ideprestserv set vlrTotalBruto = :vlrTotalBruto, vlrtotalBaseRet = :vlrtotalBaseRet, vlrtotalRetPrinc = :vlrtotalRetPrinc, vlrtotalRetAdic = :vlrtotalRetAdic, vlrtotalNRetPrinc = :vlrtotalNRetPrinc, vlrtotalNRetAdic = :vlrtotalNRetAdic  where R4010_IDREG = " + idReg.ToString();

            OracleCommand cmd = new OracleCommand(sqlUpdR4010, oc);
        ...
        {
            string sql = "select R4010.IDREG , CNPJPRESTADOR , R4010.nrInscEstab from R4010 , R4010_IDEPRESTSERV  ";
            sql = sql + " where R4010_IDEPRESTSERV.R4010_IDREG = R4010.IDREG AND nrInsc = '" + cnpjEmpresa + "000000'";
            sql = sql + " and R4010.perApur = '" + perApur + "'";
            sql = sql + " AND R4010.STATUS = 'P' ";           
        ...
        {
            string sql = "select NR_RECIBO from R4010 , R4010_IDEPRESTSERV  ";
            sql = sql + " where R4010_IDEPRESTSERV.R4010_IDREG = R4010.IDREG AND nrInsc = '" + cnpjEmpresa + "000000'";
            sql = sql + " and R4010.nrInscEstab = '" + cnpjEstabelecimento + "'";
            sql = sql + " and R4010.perApur = '" + perApur + "'";
            sql = sql + " and R4010_IDEPRESTSERV.CNPJPRESTADOR = '" + cnpjFornecedor + "'";
            sql = sql + " AND R4010.NR_RECIBO IS NOT NULL ";
            sql = sql + " AND R4010.idreg = (SELECT MAX(R4010.idreg) FROM R4010 , R4010_IDEPRESTSERV  WHERE R4010_IDEPRESTSERV.R4010_IDREG = R4010.IDREG AND  nrInsc = '" + cnpjEmpresa + "000000' AND NR_RECIBO IS NOT NULL and nrInscEstab = '" + cnpjEstabelecimento + "'  and perApur = '" + perApur + "'  and R4010_IDEPRESTSERV.CNPJPRESTADOR = '" + cnpjFornecedor + "')";
        ...
        {
            //string sqlUpdR4010 = "update r4010 set indretif = :indretif, NRRECIBO = :NRRECIBO,  nrinsc = :nrinsc, nrinscestab = :nrinscestab, INDOBRA = :INDOBRA  where idReg = " + idReg.ToString();
            string sqlUpdR4010 = "update r4010 set NRRECIBO = :NRRECIBO, INDRETIF = 2 where idReg = " + idReg.ToString();
        ...
        public void incluir(int idRegEmpresa, int indretif, string perapur, int idRegEstabelecimento, string nrRecibo, int indObra, string cnpjPrestador, int indCPRB)
        {
            ...
            string sqlUpdR2060 = "inssert into r4010 set indretif = :indretif, NRRECIBO = :NRRECIBO, perapur = :perapur, nrinsc = :nrinsc, nrinscestab = :nrinscestab, INDOBRA = :INDOBRA ";
        ...
        public void atualiza(int idReg, int idRegEmpresa, int indretif, int idRegEstabelecimento, string nrRecibo, int indObra, string cnpjPrestador, int indCPRB)
        {
            ...
            //string sqlUpdR4010 = "update r4010 set indretif = :indretif, NRRECIBO = :NRRECIBO,  nrinsc = :nrinsc, nrinscestab = :nrinscestab, INDOBRA = :INDOBRA  where idReg = " + idReg.ToString();
            string sqlUpdR4010 = "update r4010 set indretif = :indretif, INDOBRA = :INDOBRA where idReg = " + idReg.ToString();
            ...
            string sqlUpdR4010PrestServ = "update r4010_ideprestserv set CNPJPRESTADOR = :CNPJPRESTADOR, INDCPRB = :INDCPRB where R4010_IDREG = " + idReg.ToString();
            OracleCommand cmdPrestServ = new OracleCommand(sqlUpdR4010PrestServ, oc);
        ...
        public void incluirNFS(string serie, string numDocto, DateTime dtEmissaoNF, decimal vlrBruto, string obs, int R4010_IDEPRESTSERV_IDREG)
        {

            string sql = "insert into r4010_NFS (SERIE , NUMDOCTO , DTEMISSAONF , VLRBRUTO , OBS, R4010_IDEPRESTSERV_IDREG ) values " +
                "(:SERIE , :NUMDOCTO , :DTEMISSAONF , :VLRBRUTO , :OBS, " + R4010_IDEPRESTSERV_IDREG.ToString() + ")";
        ...
        public void atualizaNFS(int idReg, string serie, string numDocto, DateTime dtEmissaoNF, decimal vlrBruto, string obs, int R4010_IDEPRESTSERV_IDREG)
        {

            string sql = "update r4010_NFS set SERIE = :SERIE , NUMDOCTO = :NUMDOCTO , DTEMISSAONF = :DTEMISSAONF, VLRBRUTO = :VLRBRUTO , OBS = :obs where idReg = " + idReg.ToString();
        ...
        public void incluirTpServ(string tpServico, decimal vlrBaseRet, decimal vlrRetencao, decimal vlrRetSub, decimal vlrNRetPrinc, decimal vlrServicos15, decimal vlrServicos20, decimal vlrServicos25, decimal vlrAdicional, decimal vlrNRetAdic, int r4010_nfs_idreg)
        {

            string sql = "insert into r4010_infotpserv (TPSERVICO , VLRBASERET , VLRRETENCAO , VLRRETSUB , VLRNRETPRINC,VLRSERVICOS15, VLRSERVICOS20, VLRSERVICOS25,VLRADICIONAL, VLRNRETADIC, R4010_NFS_IDREG ) values " +
                "(:TPSERVICO , :VLRBASERET , :VLRRETENCAO , :VLRRETSUB , :VLRNRETPRINC, :VLRSERVICOS15, :VLRSERVICOS20, :VLRSERVICOS25, :VLRADICIONAL, :VLRNRETADIC, " + r4010_nfs_idreg.ToString() + ")";
        ...
        {
            string sql = "select * from R4010";
            return odb.getDataTable(sql);
        }

        public override DataTable getData(int idReg)
        {
            string sql = "select * from R4010 where idReg = " + idReg.ToString();
            return odb.getDataTable(sql);
        }

        public DataTable getDataLote(int idReg)
        {
            string sql = "SELECT * FROM R4010 CONS WHERE STATUS = 'P' AND  EXISTS (select PESQ.PERAPUR from R4010 PESQ where PESQ.PERAPUR = CONS.PERAPUR AND  PESQ.NRINSC = CONS.NRINSC AND PESQ.NRINSCESTAB = CONS.NRINSCESTAB AND PESQ.idReg = " + idReg.ToString() + ")";
            return odb.getDataTable(sql);
        }
        public DataTable getData(string cnpjEmpresa, string cnpjEstabelecimento, string perApur)
        {
            string sql = "select R4010.* , CNPJPRESTADOR,  VLRTOTALBRUTO, VLRTOTALBASERET, VLRTOTALRETPRINC from R4010 , R4010_IDEPRESTSERV  ";
            sql = sql + " where R4010_IDEPRESTSERV.R4010_IDREG = R4010.IDREG AND nrInsc = '" + cnpjEmpresa + "000000'";
        ...
        {
            string sql = "select R4010.* , CNPJPRESTADOR,  VLRTOTALBRUTO, VLRTOTALBASERET, VLRTOTALRETPRINC from R4010 , R4010_IDEPRESTSERV  ";
            sql = sql + " where R4010_IDEPRESTSERV.R4010_IDREG = R4010.IDREG AND STATUS = 'P' AND NRINSCESTAB = (SELECT CNPJ FROM ESTABELECIMENTOS WHERE CODIGO = '" + cnpjEstabelecimento + "')";
            if (cnpjFornecedor != "")
            {
        ...
        public DataTable getDataR4010IdePrestServ(int R4010_idReg)
        {
            string sql = "select * from R4010_IDEPRESTSERV where R4010_idReg = " + R4010_idReg.ToString();
            return odb.getDataTable(sql);
        }

        public DataTable getDataR4010NFS(int IdReg)
        {
            string sql = "select * from R4010_NFS where IDREG = " + IdReg.ToString();
            return odb.getDataTable(sql);
        }
        public DataTable getDataR4010NFSByR4010(int R4010_IdePrestServ_IdReg)
        {
            string sql = "select * from R4010_NFS where R4010_IDEPRESTSERV_IDREG = " + R4010_IdePrestServ_IdReg.ToString();
            return odb.getDataTable(sql);
        }

        public DataTable getDataR4010INFOTPSERV(int idReg)
        {
            string sql = "select * from R4010_INFOTPSERV where IDREG = " + idReg.ToString();
            return odb.getDataTable(sql);
        }
        public DataTable getDataR4010INFOTPSERVByNFS(int R4010_NFS_IDREG)
        {
            string sql = "select * from R4010_INFOTPSERV where R4010_NFS_IDREG = " + R4010_NFS_IDREG.ToString();
            return odb.getDataTable(sql);
        }
        public System.Collections.Generic.List<XmlDocument> getXMLEvtLote(int idReg)
        {
            ...
            for (int a = 0; a < t.Rows.Count; a++)
            {
                ...
                DataTable dt4010 = getData(Convert.ToInt32(t.Rows[a]["idREG"].ToString()));
                ...
                evt.infoServTom.ideEstabObra.tpInscEstab = Convert.ToByte(dt4010.Rows[0]["tpInscEstab"].ToString());
                evt.infoServTom.ideEstabObra.nrInscEstab = dt4010.Rows[0]["nrInscEstab"].ToString();
                evt.infoServTom.ideEstabObra.indObra = Convert.ToByte(dt4010.Rows[0]["indObra"].ToString());

                // Adicionar os Prestadores (Para o Layout 1.2 somente deve existir 1 prestador por evento)
                DataTable dt4010IdePrestServ = getDataR4010IdePrestServ(Convert.ToInt32(t.Rows[a]["idREG"].ToString()));
                evt.infoServTom.ideEstabObra.idePrestServ = new ReinfEvtServTomInfoServTomIdeEstabObraIdePrestServ();
                for (int i = 0; i < dt4010IdePrestServ.Rows.Count; i++)
                {
                    evt.infoServTom.ideEstabObra.idePrestServ = new ReinfEvtServTomInfoServTomIdeEstabObraIdePrestServ();
                    evt.infoServTom.ideEstabObra.idePrestServ.cnpjPrestador = dt4010IdePrestServ.Rows[i]["cnpjPrestador"].ToString();
                    evt.infoServTom.ideEstabObra.idePrestServ.vlrTotalBruto = Utils.ConverteNumero(dt4010IdePrestServ.Rows[i]["vlrTotalBruto"].ToString());                 
                    evt.infoServTom.ideEstabObra.idePrestServ.vlrTotalBaseRet = Utils.ConverteNumero(dt4010IdePrestServ.Rows[i]["vlrTotalBaseRet"].ToString());
                    evt.infoServTom.ideEstabObra.idePrestServ.vlrTotalRetPrinc = Utils.ConverteNumero(dt4010IdePrestServ.Rows[i]["vlrTotalRetPrinc"].ToString());
                    if (dt4010IdePrestServ.Rows[i]["vlrTotalRetAdic"].ToString() != "0" && dt4010IdePrestServ.Rows[i]["vlrTotalRetAdic"].ToString() != "")
                    {
                        evt.infoServTom.ideEstabObra.idePrestServ.vlrTotalRetAdic = Utils.ConverteNumero(dt4010IdePrestServ.Rows[i]["vlrTotalRetAdic"].ToString());
                    }
                    if (dt4010IdePrestServ.Rows[i]["vlrTotalNRetPrinc"].ToString() != "0" && dt4010IdePrestServ.Rows[i]["vlrTotalNRetPrinc"].ToString() != "")
                    {
                        evt.infoServTom.ideEstabObra.idePrestServ.vlrTotalNRetPrinc = Utils.ConverteNumero(dt4010IdePrestServ.Rows[i]["vlrTotalNRetPrinc"].ToString());
                    }
                    if (dt4010IdePrestServ.Rows[i]["vlrTotalNRetAdic"].ToString() != "0" && dt4010IdePrestServ.Rows[i]["vlrTotalNRetAdic"].ToString() != "")
                    {
                        evt.infoServTom.ideEstabObra.idePrestServ.vlrTotalNRetAdic = Utils.ConverteNumero(dt4010IdePrestServ.Rows[i]["vlrTotalNRetAdic"].ToString());
                    }
                    evt.infoServTom.ideEstabObra.idePrestServ.indCPRB = Convert.ToByte(dt4010IdePrestServ.Rows[i]["indCPRB"].ToString());


                    // Adicionar as Notas Fiscais do Prestador  
                    R4010DAL objDLL = new R4010DAL();
                    DataTable dtPrest = objDLL.getDataR4010IdePrestServ(Convert.ToInt32(t.Rows[a]["idREG"].ToString()));
                    DataTable dt4010NFS = getDataR4010NFSByR4010(Convert.ToInt16(dtPrest.Rows[i]["idReg"].ToString()));
                    evt.infoServTom.ideEstabObra.idePrestServ.nfs = new ReinfEvtServTomInfoServTomIdeEstabObraIdePrestServNfs[dt4010NFS.Rows.Count];
                    for (int j = 0; j < dt4010NFS.Rows.Count; j++)
                    {
                        evt.infoServTom.ideEstabObra.idePrestServ.nfs[j] = new ReinfEvtServTomInfoServTomIdeEstabObraIdePrestServNfs();
                        evt.infoServTom.ideEstabObra.idePrestServ.nfs[j].serie = dt4010NFS.Rows[j]["serie"].ToString();
                        evt.infoServTom.ideEstabObra.idePrestServ.nfs[j].numDocto = dt4010NFS.Rows[j]["numDocto"].ToString();
                        evt.infoServTom.ideEstabObra.idePrestServ.nfs[j].dtEmissaoNF = DateTime.Parse(dt4010NFS.Rows[j]["dtEmissaoNF"].ToString());
                        evt.infoServTom.ideEstabObra.idePrestServ.nfs[j].vlrBruto = Utils.ConverteNumero(dt4010NFS.Rows[j]["vlrBruto"].ToString());
                        if (dt4010NFS.Rows[i]["obs"].ToString() != "")
                        {
                            evt.infoServTom.ideEstabObra.idePrestServ.nfs[j].obs = dt4010NFS.Rows[j]["obs"].ToString();
                        }
                        // Adicionar os Tipos de Servicos constantes da NF
                        DataTable dt4010InfoTpServ = getDataR4010INFOTPSERVByNFS(Convert.ToInt16(dt4010NFS.Rows[j]["idReg"].ToString()));
                        evt.infoServTom.ideEstabObra.idePrestServ.nfs[j].infoTpServ = new ReinfEvtServTomInfoServTomIdeEstabObraIdePrestServNfsInfoTpServ[dt4010InfoTpServ.Rows.Count];
                        for (int k = 0; k < dt4010InfoTpServ.Rows.Count; k++)
                        {
                            ...
                            evt.infoServTom.ideEstabObra.idePrestServ.nfs[j].infoTpServ[k].tpServico = dt4010InfoTpServ.Rows[k]["tpServico"].ToString();
                            evt.infoServTom.ideEstabObra.idePrestServ.nfs[j].infoTpServ[k].vlrBaseRet = Utils.ConverteNumero(dt4010InfoTpServ.Rows[k]["vlrBaseRet"].ToString());
                            evt.infoServTom.ideEstabObra.idePrestServ.nfs[j].infoTpServ[k].vlrRetencao = Utils.ConverteNumero(dt4010InfoTpServ.Rows[k]["vlrRetencao"].ToString());
                            if (dt4010InfoTpServ.Rows[i]["vlrRetSub"].ToString() != "0")
                            {
                                evt.infoServTom.ideEstabObra.idePrestServ.nfs[j].infoTpServ[k].vlrRetSub = Utils.ConverteNumero(dt4010InfoTpServ.Rows[k]["vlrRetSub"].ToString());
                            }
                            if (dt4010InfoTpServ.Rows[i]["vlrNRetPrinc"].ToString() != "0")
                            {
                                evt.infoServTom.ideEstabObra.idePrestServ.nfs[j].infoTpServ[k].vlrNRetPrinc = Utils.ConverteNumero(dt4010InfoTpServ.Rows[k]["vlrNRetPrinc"].ToString());
                            }
                            if (dt4010InfoTpServ.Rows[i]["vlrServicos15"].ToString() != "0")
                            {
                                evt.infoServTom.ideEstabObra.idePrestServ.nfs[j].infoTpServ[k].vlrServicos15 = Utils.ConverteNumero(dt4010InfoTpServ.Rows[k]["vlrServicos15"].ToString());
                            }
                            if (dt4010InfoTpServ.Rows[i]["vlrServicos20"].ToString() != "0")
                            {
                                evt.infoServTom.ideEstabObra.idePrestServ.nfs[j].infoTpServ[k].vlrServicos20 = Utils.ConverteNumero(dt4010InfoTpServ.Rows[k]["vlrServicos20"].ToString());
                            }
                            if (dt4010InfoTpServ.Rows[i]["vlrServicos25"].ToString() != "0")
                            {
                                evt.infoServTom.ideEstabObra.idePrestServ.nfs[j].infoTpServ[k].vlrServicos25 = Utils.ConverteNumero(dt4010InfoTpServ.Rows[k]["vlrServicos25"].ToString());
                            }
                            if (dt4010InfoTpServ.Rows[i]["vlrNRetAdic"].ToString() != "0")
                            {
                                evt.infoServTom.ideEstabObra.idePrestServ.nfs[j].infoTpServ[k].vlrNRetAdic = Utils.ConverteNumero(dt4010InfoTpServ.Rows[k]["vlrNRetAdic"].ToString());
                            }
                        }
                    }
                }
        ...
        public override XmlDocument getXMLEvt(int idReg)
                {
                    ...
                    for (int a = 0; a < t.Rows.Count; a++)
                    {
                        ...
                        for (int i = 0; i < dt4010IdePrestServ.Rows.Count; i++)
                        {
                            evt.infoServTom.ideEstabObra.idePrestServ = new ReinfEvtServTomInfoServTomIdeEstabObraIdePrestServ();
                            evt.infoServTom.ideEstabObra.idePrestServ.cnpjPrestador = dt4010IdePrestServ.Rows[i]["cnpjPrestador"].ToString();
                            evt.infoServTom.ideEstabObra.idePrestServ.vlrTotalBruto = dt4010IdePrestServ.Rows[i]["vlrTotalBruto"].ToString();
                            evt.infoServTom.ideEstabObra.idePrestServ.vlrTotalBaseRet = dt4010IdePrestServ.Rows[i]["vlrTotalBaseRet"].ToString();
                            evt.infoServTom.ideEstabObra.idePrestServ.vlrTotalRetPrinc = dt4010IdePrestServ.Rows[i]["vlrTotalRetPrinc"].ToString();
                            if (dt4010IdePrestServ.Rows[i]["vlrTotalRetAdic"].ToString() != "0" && dt4010IdePrestServ.Rows[i]["vlrTotalRetAdic"].ToString() != "")
                            {
                                evt.infoServTom.ideEstabObra.idePrestServ.vlrTotalRetAdic = dt4010IdePrestServ.Rows[i]["vlrTotalRetAdic"].ToString();
                            }
                            if (dt4010IdePrestServ.Rows[i]["vlrTotalNRetPrinc"].ToString() != "0" && dt4010IdePrestServ.Rows[i]["vlrTotalNRetPrinc"].ToString() != "")
                            {
                                evt.infoServTom.ideEstabObra.idePrestServ.vlrTotalNRetPrinc = dt4010IdePrestServ.Rows[i]["vlrTotalNRetPrinc"].ToString();
                            }
                            if (dt4010IdePrestServ.Rows[i]["vlrTotalNRetAdic"].ToString() != "0" && dt4010IdePrestServ.Rows[i]["vlrTotalNRetAdic"].ToString() != "")
                            {
                                evt.infoServTom.ideEstabObra.idePrestServ.vlrTotalNRetAdic = dt4010IdePrestServ.Rows[i]["vlrTotalNRetAdic"].ToString();
                            }
                            evt.infoServTom.ideEstabObra.idePrestServ.indCPRB = Convert.ToByte(dt4010IdePrestServ.Rows[i]["indCPRB"].ToString()); ;
                            ...
                            R4010DAL objDLL = new R4010DAL();
                            DataTable dtPrest = objDLL.getDataR4010IdePrestServ(Convert.ToInt32(t.Rows[a]["idREG"].ToString()));                    
                            DataTable dt4010NFS = getDataR4010NFSByR4010(Convert.ToInt16(dtPrest.Rows[0]["idReg"].ToString()));
                            evt.infoServTom.ideEstabObra.idePrestServ.nfs = new ReinfEvtServTomInfoServTomIdeEstabObraIdePrestServNfs[dt4010NFS.Rows.Count];
                            for (int j = 0; j < dt4010NFS.Rows.Count; j++)
                            {
                                evt.infoServTom.ideEstabObra.idePrestServ.nfs[j] = new ReinfEvtServTomInfoServTomIdeEstabObraIdePrestServNfs();
                                ...
                                evt.infoServTom.ideEstabObra.idePrestServ.nfs[j].numDocto = dt4010NFS.Rows[j]["numDocto"].ToString();
                                evt.infoServTom.ideEstabObra.idePrestServ.nfs[j].dtEmissaoNF = DateTime.Parse(dt4010NFS.Rows[j]["dtEmissaoNF"].ToString());
                                evt.infoServTom.ideEstabObra.idePrestServ.nfs[j].vlrBruto = dt4010NFS.Rows[j]["vlrBruto"].ToString(); ;
                                if (dt4010NFS.Rows[i]["obs"].ToString() != "")
                                {
                                    evt.infoServTom.ideEstabObra.idePrestServ.nfs[j].obs = dt4010NFS.Rows[j]["obs"].ToString();
                                }
                                ...
                                for (int k = 0; k < dt4010InfoTpServ.Rows.Count; k++)
                                {
                                    evt.infoServTom.ideEstabObra.idePrestServ.nfs[j].infoTpServ[k] = new ReinfEvtServTomInfoServTomIdeEstabObraIdePrestServNfsInfoTpServ();
                                    evt.infoServTom.ideEstabObra.idePrestServ.nfs[j].infoTpServ[k].tpServico = dt4010InfoTpServ.Rows[k]["tpServico"].ToString(); 
                                    evt.infoServTom.ideEstabObra.idePrestServ.nfs[j].infoTpServ[k].vlrBaseRet = dt4010InfoTpServ.Rows[k]["vlrBaseRet"].ToString();
                                    evt.infoServTom.ideEstabObra.idePrestServ.nfs[j].infoTpServ[k].vlrRetencao = dt4010InfoTpServ.Rows[k]["vlrRetencao"].ToString();
                                    if (dt4010InfoTpServ.Rows[i]["vlrRetSub"].ToString() != "0")
                                    {
                                        evt.infoServTom.ideEstabObra.idePrestServ.nfs[j].infoTpServ[k].vlrRetSub = dt4010InfoTpServ.Rows[k]["vlrRetSub"].ToString();
                                    }
                                    if (dt4010InfoTpServ.Rows[i]["vlrNRetPrinc"].ToString() != "0")
                                    {
                                        evt.infoServTom.ideEstabObra.idePrestServ.nfs[j].infoTpServ[k].vlrNRetPrinc = dt4010InfoTpServ.Rows[k]["vlrNRetPrinc"].ToString();
                                    }
                                    if (dt4010InfoTpServ.Rows[i]["vlrServicos15"].ToString() != "0")
                                    {
                                        evt.infoServTom.ideEstabObra.idePrestServ.nfs[j].infoTpServ[k].vlrServicos15 = dt4010InfoTpServ.Rows[k]["vlrServicos15"].ToString();
                                    }
                                    if (dt4010InfoTpServ.Rows[i]["vlrServicos20"].ToString() != "0")
                                    {
                                        evt.infoServTom.ideEstabObra.idePrestServ.nfs[j].infoTpServ[k].vlrServicos20 = dt4010InfoTpServ.Rows[k]["vlrServicos20"].ToString();
                                    }
                                    if (dt4010InfoTpServ.Rows[i]["vlrServicos25"].ToString() != "0")
                                    {
                                        evt.infoServTom.ideEstabObra.idePrestServ.nfs[j].infoTpServ[k].vlrServicos25 = dt4010InfoTpServ.Rows[k]["vlrServicos25"].ToString();
                                    }
                                    if (dt4010InfoTpServ.Rows[i]["vlrNRetAdic"].ToString() != "0")
                                    {
                                        evt.infoServTom.ideEstabObra.idePrestServ.nfs[j].infoTpServ[k].vlrNRetAdic = dt4010InfoTpServ.Rows[k]["vlrNRetAdic"].ToString();
                                    }
                                }
                            }
                        }
